...
/*   Updated: 2024/06/19 23:27:00 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	color_choosing(int posx, int posy, char **map)
{
	int	len_max_mapx;
	int	len_max_mapy;
	int	x;
...
static void	player_vision(double x, double y, t_game *game, double number_ray)
{
	...
	mapy = (game->play_y / IMG_SIZE) + ((ray_y) / ((double)WIN_HEIGHT / 54.));
	mapx = (game->play_x / IMG_SIZE) + ((ray_x) / ((double)WIN_HEIGHT / 54.));
		...
		&& x + ray_x > 0 && y + ray_y > 0)
	{
		...
		mapy = (game->play_y / IMG_SIZE) + (ray_y / ((double)WIN_HEIGHT / 54.));
		mapx = (game->play_x / IMG_SIZE) + (ray_x / ((double)WIN_HEIGHT / 54.));
		my_mlx_pixel_put(&game->img, x + ray_x, y + ray_y, JAUNE);
		nbr_pixel++;
	}
}
...
static void	print_layout(t_game *game, double posx, double posy, char **map)
{
	...
	while (y < game->minimap_size)
	{
		...
		while (x < game->minimap_size)
		{
			...
			color = color_choosing((posx + pxl_x), (posy + pxl_y), map);
			if (color == 1)
				my_mlx_pixel_put(&game->img, x, y, BLANC);
			else if (color == 0)
				my_mlx_pixel_put(&game->img, x, y, NOIR);
			else
				my_mlx_pixel_put(&game->img, x, y, MARRON);
			x++;
		}
		y++;
	}
