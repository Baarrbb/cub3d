...
/*   Updated: 2024/06/06 18:11:44 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_wall(t_game *game, int slice, int pos, int flag)
{
	...
	while (++i < slice)
	{
		...
		if (FOG)
		{
			...
			game->intensity = fmax((game->distance - (int)IMG_SIZE) / game->distance, 0.1);
			int	r = 100;
			
			if (game->intensity == 0.1)
				color = darken_color(color, game->intensity);
			else if (((pos - (WIN_WIDTH / 2)) * (pos - (WIN_WIDTH / 2))) + ((y - (WIN_HEIGHT / 2)) * (y - (WIN_HEIGHT / 2))) < r*r)
			{
...
static void	render_floor(t_game *game, int slice, int pos)
{
	...
	{
		color = game->ceiling;
		// game->intensity = 0.88;//(game->distance - (int)IMG_SIZE) / game->distance;
		// if (game->distance > (int)IMG_SIZE)
		// int r = 100;
		// if (((pos - (WIN_WIDTH / 2)) * (pos - (WIN_WIDTH / 2))) + ((y - (WIN_HEIGHT / 2)) * (y - (WIN_HEIGHT / 2))) < r*r)
		// 	color = darken_color(color, 0);
		// else
			color = darken_color(game->ceiling, 0.88);
		if (FOG)
			my_mlx_pixel_put(&game->img, pos, y, color);
		else
	...
	while (y < WIN_HEIGHT)
	{
		...
		if (((pos - (WIN_WIDTH / 2)) * (pos - (WIN_WIDTH / 2))) + ((y - (WIN_HEIGHT / 2)) * (y - (WIN_HEIGHT / 2))) < r*r)
		{
			if (game->intensity - 0.25 > 0)
				...
				color = game->floor;
			// color = darken_color(color, game->intensity / 1.5);
		}
		else
